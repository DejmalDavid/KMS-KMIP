...
from kmip.etcd3.ETCDwrapper import ETCDwrapper
...
class KmipEngine(object):
    ...
    def __init__(self, policies=None, database_path=None):
        ...
        """
        #EDIT BY DAVID DEJMAL
        #main flag for usage etcd or SQLite
        if database_path == "etcd3":
            self.etcd=True
        else:
            self.etcd=False

        if self.etcd:
            self.client = ETCDwrapper('127.0.0.1',2379)
            self.client.connect() 
        else:
    ...
    def _get_object_type(self, unique_identifier):
        ...
        if self.etcd :
            try:
                object_type = self.client.find(unique_identifier)
            except Exception as e:
                self._logger.warning("Error with finding:".format(unique_identifier))
                raise e
        else:
            try:
    ...
    def _get_object_with_access_controls(
        ...
        if self.etcd :
            try:
                managed_object = self.client.find(uid)
            except Exception as e:
                self._logger.warning("Error with finding:".format(uid))
                raise e
        else:
    ...
    def _list_objects_with_access_controls(
        ...
        #EDITED by David Dejmal
        if self.etcd:
            managed_objects = self.client.find_all()
        else:
    ...
    def _process_create(self, payload):
        ...
        #EDITED by David Dejmal
        if self.etcd:
            self.client.add(managed_object)  
        else:
    ...
    def _process_create_key_pair(self, payload):
        ...
        #EDITED by David Dejmal
        if self.etcd:
            self.client.add(public_key)
            self.client.add(private_key)
        else:
    ...
    def _process_delete_attribute(self, payload):
        ...
        #EDITED by David Dejmal
        if self.etcd:
            self.client.update(managed_object.unique_identifier,managed_object)
        else:
            self._data_session.commit()
        #END
    ...
    def _process_destroy(self, payload):
        ...
        #EDITED by David Dejmal
        if self.etcd:
            self.client.delete(unique_identifier)
        else:
